...
const routeSlice = createSlice({
  ...
  reducers: {
    changeRoute(state, action) {
      ...
      const nonmatchedRoutes = routes.filter(
        (route) => route !== matchedRoute[0]
      );
      const falsifiedNonmatches = nonmatchedRoutes.map((route) => ({
        [route]: false,
      }));
      ...
      return Object.assign({ [matchedRoute[0]]: true }, ...falsifiedNonmatches);
    },
  },
});
