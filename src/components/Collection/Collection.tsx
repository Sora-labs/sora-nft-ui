...
export const Collection = (
  ...
  return (
    <div className="w-full md:w-auto md:my-4 md:rounded-2xl flex flex-col justify-center md:mx-8 px-3 pt-5 relative overflow-hidden box-border">
      <div
        className="absolute top-0 right-0 bottom-0 left-0 md:mx-8 bg-cover bg-no-repeat bg-center blur-md bg-light-gray-100 scale-110"
      ...
      <div className="collection-wrapper">
        <div className="relative z-10 2xl:w-full flex gap-5 mb-4 px-6">
          <div className="flex 2xl:flex-col flex-1 gap-8">
            ...
            <div className="w-full 2xl:w-auto flex flex-col gap-4 justify-center">
              <div className="text-xl 2xl:text-3xl text-white font-semibold">
                {collectionName}
              </div>
              <div
                className="text-lg 2xl:text-xl text-white font-medium"
                aria-label="nfts in collection"
              >
                {collectionItems.length} NFTs
              </div>
            </div>
          </div>
        ...
        <div
          className="grid grid-col-3 gap-8 overflow-x-auto overflow-y-hidden hide-scrollbar pb-4 px-9 md:px-6 box-border"
          style={{
        ...
        >
          {collectionItems.map((i, index) => (
            <Link
              to={`/${index}`}
              key={index}
              className="nft-card flex gap-4"
              aria-label="nft card"
              style={{ scrollSnapAlign: "start" }}
            >
              <div className="flex nft-card flex-col flex-1 rounded-md md:rounded-lg overflow-hidden">
                <div
                  className="flex relative w-full justify-center items-center overflow-hidden"
                  style={{ scrollSnapAlign: "start" }}
                ...
                <div
                  className="relative bottom-0 z-10 dark:border-dark-gray-50 px-4 py-4 bg-white dark:bg-dark-gray-90"
