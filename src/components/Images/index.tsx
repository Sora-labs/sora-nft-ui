...
export const ImageWithLoader = (
  ...
  const { src, name, width, height, rounded, padding } = props;
    ...
    <BaseImage width={width} height={height} padding={padding}>
      {isLoad && (
        <div className="absolute inset-0 z-10 w-full h-full flex items-center justify-center bg-light-gray-45 dark:bg-dark-gray-80">
          <AiOutlineLoading3Quarters className="text-4xl animate-spin"></AiOutlineLoading3Quarters>
        </div>
      )}
    ...
    >
      {props.children ? (
        ...
        <img
          src={src}
          alt={name ? name : "not found"}
          className={`w-full block ${rounded ? rounded : "rounded-sm"}`}
        />
      )}
    </div>
  );
